		...
		public String Login(
			...
			case OK:
				...
				if(inputData.getMemDivision().equals("H02")) {
					...
					session.setAttribute("authNm", inputData.getUsers().getUsersNm()); 
					session.setAttribute("authDivision", inputData.getMemDivision()); 
					log.info("이름:{}",inputData.getUsers().getUsersNm());
					
				}else if(inputData.getMemDivision().equals("H03")) {
					viewName = "redirect:/indexCompany";
					session.setAttribute("authId", inputData.getMemId()); 
					session.setAttribute("authNm", inputData.getCompany().getCompanyDnm()); 
					session.setAttribute("authDivision", inputData.getMemDivision()); 
				}else if(inputData.getMemDivision().equals("H01")) {
					viewName = "redirect:/indexAdmin";
					session.setAttribute("authId", inputData.getMemId()); 
					session.setAttribute("authNm", "관리자"); 
				}
				break;
