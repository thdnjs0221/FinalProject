...
$(function(){
	...
	$('#resumeAutoInsert').on("click",function(){
		...
		// 경력
		$("[name='careerVOList[0].crCompanynm']").val("(주)우아한형제들");
		$("[name='careerVOList[0].crStdate']").val("20210319");
		$("[name='careerVOList[0].crEndate']").val("20230315");
		$("[name='careerVOList[0].crContent']").val("배민커머스서비스실 배민커머스웹프론트개발팀 프론트엔드 개발자로 근무");
		
		//학력
		$("[name='educationVOList[0].eduSchoolnm']").val("충남대학교");
		$("[name='educationVOList[0].eduMajornm']").val("컴퓨터융합학부");
		...
		//경험/활동/교육
		$("[name='intrnActvtVOList[0].iaName']").val("네이버클라우드 주식회사");
		$("[name='intrnActvtVOList[0].iaStdate']").val("20181217");
		$("[name='intrnActvtVOList[0].iaEndate']").val("20190111");
		$("[name='intrnActvtVOList[0].iaContent']").val("Vision LLM 모델 개발을 위한 데이터 업무 인턴십(체험형)");
		
// 		$("[name='intrnActvtVOList[1].iaName']").val("2017 청년 자원봉사 「녹색 청년회」");
// 		$("[name='intrnActvtVOList[1].iaStdate']").val("20170731");
// 		$("[name='intrnActvtVOList[1].iaEndate']").val("20170813");
// 		$("[name='intrnActvtVOList[1].iaContent']").val("봉사활동");
		
		//자격증
		$("[name='certificateVOList[0].cerNm']").val("정보처리기사");	
		...
		$("[name='coverLetterVOList[0].psContent']").val(
				"[실패는 나의 경험치] \n배움에 대해 돈을 아끼지 않는 부모님 밑에서 자랐습니다. 여러 예체능 학원에 다니면서 다양한 경험을 쌓으며 제가 겪은 실패들은 모두 성장을 위한 경험으로 남았습니다. 학원 바둑 대회에서 패배하고 올 때면 항상 부모님께서는 ‘패배도 경험이다.’라는 말을 해주며 저에게 나아갈 방향을 알려주셨습니다. 이런 실패를 계기로 부족한 부분을 보완하고 개선해 나가는 과정에서 더욱 발전할 수 있었습니다. 대학교 입학 후 첫 시험에 3.3의 학점을 얻었습니다. 저는 제 성적에 실망했고, 장학금을 받기에는 턱없이 부족했습니다. 이 악물고 공부하여 성적은 점점 올라갔으며 마침내 1등의 성적으로 장학금을 받았습니다. 해당 경험 처음의 실패에 좌절하지 않고 더욱 노력해 나가면 달성할 수 있다는 것을 배웠습니다. 그리고 학점에 관한 기준은 그저 숫자일 뿐 그 의미는 어떠한 부분에서도 할 수 있다는 자신감을 주었습니다.");
		
		$("[name='coverLetterVOList[1].psTitle']").val("성격의 장단점");
		$("[name='coverLetterVOList[1].psContent']").val("저의 장점은 다양한 생각에 포용적입니다. 사람들과 소통하면서 의견을 주고받는 것을 좋아합니다. 제가 이런 장점을 가질 수 있었던 이유는 교내에서 다양한 프로젝트를 진행하며 다양한 가치관을 가진 친구들과 회의하며 의견을 맞춰간 경험이 있기 때문입니다. 이러한 경험은 입사 후 팀원들과 프로젝트를 진행할 때 협업하며 잘 융화되어 시너지 효과를 낼 수 있을 것입니다. \n저의 단점은 목표나 성취해야 할 일이 없으면 느슨해지는 것입니다. 그래서 방황하는 시간이 길어졌으며 이것을 다음을 위한 재충전의 시간이라고 스스로 합리화했습니다. 하지만 이런 저를 되돌아보면서 시간을 헛되게 보내는 것에 대해 인지하였고 내부의 목표를 정해 실천하고 있습니다. 하루에 정해진 시간만큼은 꼭 자기개발을 진행하며 오늘의 일을 내일로 미루지 않도록 노력하고 있습니다.");
	
		//제목
		$("[name='resumeTitle']").val("ooo_이력서");
